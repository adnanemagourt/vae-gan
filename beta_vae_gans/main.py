"""
Main training script for β-VAE-GAN model
"""
...
def train(model, train_loader, val_loader, config, logger):
    ...
    temp_real_dir = os.path.join(tempfile.gettempdir(), 'vae_gan_real')
    temp_fake_dir = os.path.join(tempfile.gettempdir(), 'vae_gan_fake')
    ...
    for epoch in range(config.epochs):
        ...
        for i, images in pbar:
            ...
            d_loss_dict = model.loss_function(
                images, recon_images, mu, logvar, disc_real, disc_fake, train_generator=False
            )
            ...
            g_loss_dict = model.loss_function(
                images, recon_images, mu, logvar, disc_real, disc_fake, train_generator=True
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train β-VAE-GAN model')
    parser.add_argument('--output_dir', type=str, default='./output/beta_vae_gan',
